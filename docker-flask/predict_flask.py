...
# Obtener la información de conexión de las variables de entorno
mongo_host = os.environ.get('MONGO_HOST', 'localhost')  # 'localhost' como valor por defecto
mongo_port = int(os.environ.get('MONGO_PORT', 27017))    # 27017 como valor por defecto
...
FLIGHT_DELAY_ML_RESPONSE_TOPIC = 'flight-delay-ml-response' # Added for SSE Kafka responses
...
@app.route('/flights/delays/predict/kafka_event_stream/<unique_id>')
def kafka_event_stream(unique_id):
    def generate_kafka_events():
        consumer = None
        try:
            consumer = KafkaConsumer(
                FLIGHT_DELAY_ML_RESPONSE_TOPIC,
                bootstrap_servers=['kafka:9092'], # Ensure this matches your Kafka setup
                auto_offset_reset='latest',
                consumer_timeout_ms=20000, # Timeout after 20 seconds of no messages
                value_deserializer=lambda m: json.loads(m.decode('utf-8')),
                api_version=(0,10) # Match producer API version if set
            )
            app.logger.info(f"SSE Kafka Consumer created for topic '{FLIGHT_DELAY_ML_RESPONSE_TOPIC}' and UUID '{unique_id}'")
            
            found_message = False
            for message in consumer: # This loop will run until consumer_timeout_ms
                app.logger.debug(f"SSE Kafka Consumer received message: {message.value} for UUID '{unique_id}' (searching)")
                if isinstance(message.value, dict) and message.value.get('UUID') == unique_id:
                    app.logger.info(f"SSE Kafka Consumer found matching message for UUID '{unique_id}': {message.value}")
                    yield f"data: {json.dumps(message.value)}\n\n"
                    found_message = True
                    break # Exit loop once the message is found and sent
            
            if not found_message:
                app.logger.info(f"SSE Kafka Consumer: No matching message found for UUID '{unique_id}' within timeout.")

        except Exception as e:
            app.logger.error(f"SSE Kafka Consumer error for UUID '{unique_id}': {e}", exc_info=True)
            try:
                yield f"data: {json.dumps({'error': str(e), 'UUID': unique_id, 'status': 'ERROR'})}\n\n"
            except Exception as E_json:
                app.logger.error(f"SSE Kafka Consumer: Failed to json.dumps error message: {E_json}")
        finally:
            if consumer:
                app.logger.info(f"SSE Kafka Consumer closing for UUID '{unique_id}'.")
                consumer.close()
            
    return Response(generate_kafka_events(), mimetype='text/event-stream')
